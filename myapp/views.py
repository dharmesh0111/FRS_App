...
def login_page(request):
    if request.method == "GET":
        ...
        if user is None:
            ...
            if str(request.user) == "approvalUser1":
                return redirect("/invoices/")
            if str(request.user) == "approvalUser2":
                return redirect("/invoices/approval2/")
            else:
                messages.info(request, 'user dont have access!!')
                return redirect("/login")
...
def invoice(request):
    if request.user.is_authenticated:
        ...
        messages.warning(request, "Please log in to access the invoice page.")
        return redirect('/login')
...
def statusAPP2(request, id, code):
    ...
    messages.success(request, mess) if code == "1" else messages.warning(request, mess)
...
def aproval2(request):
    if request.user.is_authenticated:
        if(str(request.user) =="approvalUser2"):
            invoices = Invoice.objects.filter(approval1 = "success")
            return render(request,"approval2.html",{'invoices': invoices})
        else:
            messages.success(request, "approval2 only has the access")
            return redirect("/")
...
def comment(request, id):
    ...
    if request.method == "POST":
        ...
        if user_comment:  # Ensure comment is not empty
            ...
            invoice.save()
            messages.success(request, "Comment updated successfully for Approval 1.")
        else:
            messages.warning(request, "Comment cannot be empty for Approval 1.")
        ...
        if user_comment:  # Ensure comment is not empty
            ...
            invoice.save()
            messages.success(request, "Comment updated successfully for Approval 2.")
        else:
            messages.warning(request, "Comment cannot be empty for Approval 2.")
    ...
    if request.method == "POST":
        ...
        if user_subamt:  # Ensure the value is not empty
            try:
                ...
                invoice.save()
                messages.success(request, "Sub Total updated successfully!")
            except ValueError:
                messages.error(request, "Invalid input. Please enter a valid number.")
        else:
            messages.warning(request, "Sub Total cannot be empty.")
